        ...
        <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2.5rem' }}>
            <div style={{ width: '52px', height: '52px', borderRadius: '14px', background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <UnifiedIcon name="Monitor" size={26} color="white" />
            </div>
            <div>
          ...
          <div style={{ background: 'var(--bg-card)', padding: 'var(--space-card)', borderRadius: '24px', border: '1px solid var(--border)', boxShadow: 'var(--shadow-lg)' }}>
            ...
            {error && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                ...
                <UnifiedIcon name="Danger" size={18} color="#dc2626" />
            ...
            {require2FA ? (
              <motion.form initial={{ opacity: 0 }} animate={{ opacity: 1 }} onSubmit={handleSubmit}>
                ...
                <button type="submit" className="btn-primary" style={{ width: '100%', padding: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }} disabled={loading}>
                  {loading ? <span className="spinner" style={{ width: '18px', height: '18px' }} /> : <><UnifiedIcon name="Login" size={18} /> Verify & Login</>}
                </button>
              </motion.form>
            ) : (
              <form onSubmit={handleSubmit}>
                <div style={{ marginBottom: '1.25rem' }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    <div style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8', display: 'flex' }}>
                        <UnifiedIcon name="Direct" size={16} />
                    </div>
                ...
                <button type="submit" className="btn-primary" style={{ width: '100%', padding: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }} disabled={loading}>
                  {loading ? <span className="spinner" style={{ width: '18px', height: '18px' }} /> : <><UnifiedIcon name="Login" size={18} /> Sign In</>}
                </button>
              </form>
            )}
