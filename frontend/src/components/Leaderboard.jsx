...
const Leaderboard = () => {
  ...
  const getRankStyle = (rank) => {
    switch (rank) {
      case 1: return { bg: 'linear-gradient(135deg, #FFD700, #FDB931)', color: 'white', icon: <UnifiedIcon name="Ranking" size={20} /> };
      case 2: return { bg: 'linear-gradient(135deg, #C0C0C0, #A9A9A9)', color: 'white', icon: <UnifiedIcon name="Award" size={20} /> };
      case 3: return { bg: 'linear-gradient(135deg, #CD7F32, #B87333)', color: 'white', icon: <UnifiedIcon name="Award" size={20} /> };
  ...
  if (loading) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center', color: '#94a3b8' }}>
        <div style={{ marginBottom: '1rem', display: 'flex', justifyContent: 'center' }} className="spin">
            <UnifiedIcon name="MagicStar" size={24} />
        </div>
    ...
    <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '1.5rem', border: '1px solid var(--border)', boxShadow: 'var(--shadow)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
        <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: 'var(--primary-light)20', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--primary)' }}>
          <UnifiedIcon name="Award" size={24} />
        </div>
        <div>
            ...
            >
              ...
              <div style={{ marginLeft: '1rem', flex: 1 }}>
                ...
                <div style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <UnifiedIcon name="People" size={10} /> {item.total_referrals} Referrals
                </div>
              </div>
      ...
      <div style={{ marginTop: '1.5rem', padding: '1rem', background: 'var(--bg)', borderRadius: '12px', border: '1px dashed var(--border)', textAlign: 'center' }}>
        <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', margin: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.3rem' }}>
          <UnifiedIcon name="MagicStar" size={12} color="#FFD700" />
